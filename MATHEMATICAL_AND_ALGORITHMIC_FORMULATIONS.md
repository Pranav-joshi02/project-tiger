...
Given an anchor feature $\mathbf{z}_a \in \mathbb{R}^D$, a positive feature $\mathbf{z}_p \in \mathbb{R}^D$ (same individual), a negative feature $\mathbf{z}_n \in \mathbb{R}^D$ (different individual), and margin $\alpha > 0$:
...
$$\text{Block}_n = \left\{ n, \, t_n, \, H_{\text{prev}} = \operatorname{SHA-256}(\text{Block}_{n-1}), \, H_{\text{root}}^{(n)}, \, \operatorname{HMAC}_{K}(n \,\|\, H_{\text{prev}} \,\|\, H_{\text{root}}^{(n)}) \right\}$$
